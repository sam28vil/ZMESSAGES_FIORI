sap.ui.define([
	...
	return BaseController.extend("cristianofiori.cadastroclientes.controller.Worklist", {
		...
		onClienteDelete: function (oEvent) {
			...
			//remove mensagens anteriores do messagemanager
			...
			for (var i = 0; i < aItens.length; i++) {
				...
				this.getView().getModel().remove(aItens[i], {
					success: function (OData, response) {
						debugger
						// extrair as mensagens
						var mensagem = JSON.parse(response.headers["sap-message"]);
		...
		_getMessagePopover: function () {
			...
			// cria popover
			// se não tem instancia, cria instancia
			if (!this._pMessagePopover) {
				this._pMessagePopover = sap.ui.core.Fragment.load({
					id: oView.getId(),
					name: "cristianofiori.cadastroclientes.view.MessagePopover"
